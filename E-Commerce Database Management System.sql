--creating a department table called dp having 3 columns--
CREATE TABLE DP(
DP_ID INT PRIMARY KEY,
DP_NAME VARCHAR(34),
LOCATION_ VARCHAR(54)
);
--creating a employees table called EMPY having 3 columns--
CREATE TABLE EMPY(
...
--creating a projects table called PROJS having 4 columns--
CREATE TABLE PROJS(
...
--creating a assignment table called ASSIGN having 4 columns--
CREATE TABLE ASSIGN(
...
--creating a clients table called CLS having 3 columns--
CREATE TABLE CLS(
CL_ID INT PRIMARY KEY,
CL_NAME VARCHAR(34),
CONTACT_INFO BIGINT
);
--creating a contracts table called CONTRACTS having 4 columns--
...
--creating a teams table called TEAMS having 3 columns--
CREATE TABLE TEAMS(
...
--creating a team members table called TEAM_MEM  having 3 columns--
...
--Insertion of real data into DP table--
INSERT INTO DP(DP_ID,DP_NAME,LOCATION_)
VALUES(7,'IT', 'LUCKNOW'),
(4,'FINANCE','DELHI'),
(9, 'BUSINESS', 'AGRA');

--Insertion of real data into EMPY table--
...
--INSERTION IN PROJECTS (PROJS) TABLE--
...
--Insertion of real data into   ASSIGN table--
...
--Insertion of real data into  CONTRACTS table--
 INSERT INTO CONTRACTS( CON_ID, CL_ID, PROJ_ID, CONT_VALUE) 
 VALUES(88,76,4, 87787),
 (89,43,2,65643),
 (90, 75, 1, 98933),
 (91, 31, 5,73498);
 --Insertion of real data into TEAMS table--
...
--Insertion of real data into TEAM_MEM table--
 ...
 VALUES( 15, 52, 98725);
  ...
  -- Name the department and the client who assigned the project as well as the project name--
...
-- How many total hours are spent on a project? Name them with Project IDS--
SELECT SUM(HRS_WORKED) AS _TOTAL_HOURS_ , PROJ_ID
FROM ASSIGN
GROUP BY PROJ_ID;
 SELECT *FROM ASSIGN;
 --List all employees who are team leads along with their team names --
...
SELECT *FROM TEAMS;
-- What is the total contract value managed by each department?--
...
-- What are the names of the employees who have worked the most hours on a single project? --
...
SELECT *FROM CONTRACTS;
SELECT * FROM PROJS;
-- Retrieve all the projects that have a budget greater than the total contract value assigned to them --
...
-- Trigger to prevent null contract value(CONT_VALUE) but checks if there's a row with null project ID if there's any null value then it raises an error message and rollbacks --
...
--Creative view that displays all employees who have a salary greater than 50,000 including their employee names(emp_name), department (dp_name) and salary --
...
-- A stored procedure which Update salary of employees taking @EMP_ID and @percentageIncrease as parameters (variables) along with printing the message "salary updated successfully"--
...
-- A stored procedure to get employee names and their salaries by department id passed as parameter--
...
END;
--EXECUTION OF GetEmployeesByDepartment PROC--
...
END;
--EXECUTION OF GETNAMESANDSALARY PROC--
 ...
 UPDATE EMPY SET SALARY = 87800 WHERE EMP_NAME = 'FHJ JDHF' ;
  ...
  --CREATE A PROCEDURE TO FIND THE TOTAL SALARIES OF ALL EMPLOYEES IN A DEPARTMENT BY PASSING DEPARTMENT ID AS PARAMETER--
...
--EXECUTION OF TOTALSALARIESOFALLDEPT PROC--
  ...
  END;
--EXECUTION OF HIGHESTSALARYIDEPT PROC--
...
--Inserting real data into CUSTOMERS table--
...
--Inserting more real data into CUSTOMERS table--
INSERT INTO customers (customer_name, email, phone_no, address_)
VALUES
...
--Inserting even more real data into CUSTOMERS table--
INSERT INTO customers (customer_name, email, phone_no, address_)
VALUES
...
---Creating a PRODUCTS table with 6 columns--
...
--Inserting real data into PRODUCTS table--
...
--Inserting more real data into PRODUCTS table--
INSERT INTO products (product_id, product_name, arrival_date, stock_quantity, category, price)
VALUES
...
--Inserting even more real data into PRODUCTS table--
INSERT INTO products (product_id, product_name, arrival_date, stock_quantity, category, price)
VALUES
...
--Creating a SALES table with 6 columns and foreign keys referencing CUSTOMERS and PRODUCTS tables--
CREATE TABLE sales (
...
SELECT *FROM SALES;
--Inserting real data into SALES table--
...
--Inserting even more real data into SALES table--
INSERT INTO sales (sales_id, customer_id, product_id, quantity_sold, sale_date, total_amount)
VALUES
...
--Creating a TRANSACTIONS table with 7 columns and foreign keys referencing CUSTOMERS and PRODUCTS tables--
...
--Selecting all from TRANSACTIONS table--
SELECT*FROM TRANSACTIONS;
--Inserting real data into TRANSACTIONS table--
...
--Inserting even more real data into TRANSACTIONS table--
INSERT INTO transactions (transaction_id, customer_id, product_id, total_price, transaction_date, payment_method, status_) VALUES
...
--Query to find total sales for each month--
...
BEGIN
 ...
 END;
 END
--Creating another procedure to insert a new record into the customer table--
